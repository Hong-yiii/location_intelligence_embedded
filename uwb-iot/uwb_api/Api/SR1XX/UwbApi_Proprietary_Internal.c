...
#endif // UWBFTR_CCC
        default:
            NXPLOG_UWBAPI_E("%s: unknown param Id 0x%X", __FUNCTION__, paramId);
            return FALSE;
        }
    }
    return TRUE;
...
static tUWBAPI_STATUS setDefaultAoaCalibration()
{
    ...
    if (phNxpUciHal_GetNxpNumValue(UWB_AOA_CONFIG_BLOCK_COUNT, &count, sizeof(count))) {
        LOG_D("%s : Number of AOA calibration config count is %d", __FUNCTION__, count);
        for (int i = 0; i < count; i++) {
            if ((phNxpUciHal_GetNxpByteArrayValue(aoa_config_block_names[i], &p_cmd, &cmd_len) == TRUE) &&
                cmd_len > 0) {
 ...
 **
 *******************************************************************************/
void processProprietaryNtf(uint8_t event, uint16_t responsePayloadLen, uint8_t *responsePayloadPtr)
{
    switch (event) {
 ...
 **
 *******************************************************************************/
void processVendorNtf(uint8_t event, uint16_t responsePayloadLen, uint8_t *responsePayloadPtr)
{
    switch (event) {
    ...
    case VENDOR_UCI_MSG_URSK_DELETION_REQ: {
        phOsalUwb_SetMemory(&uwbContext.rsp_data, 0x00, MAX_UCI_PACKET_SIZE);
        if ((responsePayloadLen > 0) && (responsePayloadLen <= sizeof(uwbContext.rsp_data))) {
            uwbContext.rsp_len = responsePayloadLen;
            UWB_STREAM_TO_ARRAY(uwbContext.rsp_data, responsePayloadPtr, responsePayloadLen);
        }
    } break;
#endif //(UWBFTR_SE_SN110)
 ...
 **
 *******************************************************************************/
void processProprietarySeNtf(uint8_t event, uint16_t responsePayloadLen, uint8_t *responsePayloadPtr)
{
    switch (event) {
 ...
 **
 *******************************************************************************/
void processInternalNtf(uint8_t event, uint16_t responsePayloadLen, uint8_t *responsePayloadPtr)
{
    switch (event) {
#if UWBFTR_UWBS_DEBUG_Dump
...
void extDeviceManagementCallback(uint8_t gid, uint8_t event, uint16_t paramLength, uint8_t *pResponseBuffer)
{
    ...
    if ((paramLength > UCI_RESPONSE_STATUS_OFFSET) && (pResponseBuffer != NULL)) {
        ...
        case UCI_GID_PROPRIETARY:
            processProprietaryNtf(event, responsePayloadLen, responsePayloadPtr);
            break;
        case UCI_GID_VENDOR:
            processVendorNtf(event, responsePayloadLen, responsePayloadPtr);
            break;
        case UCI_GID_PROPRIETARY_SE:
            processProprietarySeNtf(event, responsePayloadLen, responsePayloadPtr);
            break;
        case UCI_GID_INTERNAL_GROUP:
            processInternalNtf(event, responsePayloadLen, responsePayloadPtr);
            break;
        }
