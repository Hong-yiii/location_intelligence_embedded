...
void USART_UserCallback(USART_Type *base, usart_dma_handle_t *handle, status_t status, void *userData)
{
    ...
    if (kStatus_USART_RxIdle == status) {
        ...
        if (gRxBuffer[MCTT_GID_INDEX] == MCTT_SE_GID_HEARDER) {
            payloadLength = gRxBuffer[MCTT_SE_PAYLOD_LENGTH_INDEX];
        }
        else if (gRxBuffer[MCTT_GID_INDEX] == MCTT_DATA_SEND) {
            payloadLength = gRxBuffer[MCTT_SE_PAYLOD_LENGTH_INDEX] + (gRxBuffer[MCTT_SE_PAYLOD_LENGTH_INDEX + 1] << 8);
        }
        else {
