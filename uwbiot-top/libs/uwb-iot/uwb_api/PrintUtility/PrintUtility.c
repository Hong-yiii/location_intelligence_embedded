...
EXTERNC void printSessionStatusData(const phUwbSessionInfo_t *pSessionInfo)
{
    if (pSessionInfo != NULL) {
        NXPLOG_APP_D("pSessionInfo->sessionHandle          : %" PRIu32 "", pSessionInfo->sessionHandle);
        NXPLOG_APP_D("pSessionInfo->state               : %hu", pSessionInfo->state);
        NXPLOG_APP_D("pSessionInfo->reason_code         : %hu", pSessionInfo->reason_code);
    }
    else {
...
EXTERNC void printUwbSessionData(const phUwbSessionsContext_t *pUwbSessionsContext)
{
    if (pUwbSessionsContext != NULL) {
        NXPLOG_APP_D("Status                        : %" PRIu32 " ", pUwbSessionsContext->status);
        NXPLOG_APP_D("Session Counter               : %d ", pUwbSessionsContext->sessioncnt);
        for (uint8_t i = 0; i < pUwbSessionsContext->sessioncnt; i++) {
            ...
            NXPLOG_APP_D("Session %d Type           : %hu", i, pUwbSessionsContext->pUwbSessionData[i].session_type);
            NXPLOG_APP_D("Session %d State          : %hu", i, pUwbSessionsContext->pUwbSessionData[i].session_state);
        }
    }
    else {
...
EXTERNC void printMulticastListStatus(const phMulticastControleeListNtfContext_t *pControleeNtfContext)
{
    if (pControleeNtfContext != NULL) {
        ...
        NXPLOG_APP_D("pControleeNtfContext->remaining_list      : %hu", pControleeNtfContext->remaining_list);
        NXPLOG_APP_D("pControleeNtfContext->no_of_controlees    : %hu", pControleeNtfContext->no_of_controlees);

        for (uint8_t i = 0; i < pControleeNtfContext->no_of_controlees; i++) {
#if UWBIOT_UWBD_SR1XXT
            NXPLOG_APP_D("pControleeNtfContext->controleeStatusList[%hu].controlee_mac_address  : %" PRIu16 "",
                i,
                pControleeNtfContext->controleeStatusList[i].controlee_mac_address);
#endif
            NXPLOG_APP_D("pControleeNtfContext->controleeStatusList[%hu].subsession_id          : %" PRIu32 "",
                i,
                pControleeNtfContext->controleeStatusList[i].subsession_id);
            NXPLOG_APP_D("pControleeNtfContext->controleeStatusList[%hu].status                 : %hu",
                i,
...
{
    if (pRcvDataPkt != NULL) {
        NXPLOG_APP_D("pRcvDataPkt->sessionHandle            : 0x%X", pRcvDataPkt->sessionHandle);
        NXPLOG_APP_D("pRcvDataPkt->status                   : 0x%X", pRcvDataPkt->status);
...
EXTERNC void printCccRangingData(const phCccRangingData_t *cccRangingData)
{
    if (cccRangingData != NULL) {
        ...
        NXPLOG_APP_D("CCC.sessionHandle                             : 0x%x ", cccRangingData->sessionHandle);
        NXPLOG_APP_D("CCC.rangingStatus                             : 0x%.2X", cccRangingData->rangingStatus);
        NXPLOG_APP_D("CCC.stsIndex                                  : %" PRIu32 "", cccRangingData->stsIndex);
        NXPLOG_APP_D("CCC.rangingRoundIndex                         : %" PRIu16 "", cccRangingData->rangingRoundIndex);
        NXPLOG_APP_I("CCC.distance                                  : %" PRIu16 "", cccRangingData->distance);
        NXPLOG_APP_D("CCC.uncertanityAnchorFom                      : %hu", cccRangingData->uncertanityAnchorFom);
        NXPLOG_APP_D("CCC.uncertanityInitiatorFom                   : %hu", cccRangingData->uncertanityInitiatorFom);
        LOG_MAU8_D("CCC.ccmTag                                    ", cccRangingData->ccmTag, MAX_CCM_TAG_SIZE);
        NXPLOG_APP_D("CCC.aoa_azimuth                               : %d.%d", TO_Q_9_7(cccRangingData->aoa_azimuth));
        NXPLOG_APP_D("CCC.aoa_azimuth_fom                           : %d", cccRangingData->aoa_azimuth_FOM);
        NXPLOG_APP_D("CCC.aoa_elevation                             : %d.%d", TO_Q_9_7(cccRangingData->aoa_elevation));
        NXPLOG_APP_D("CCC.aoa_elevation_fom                         : %d", cccRangingData->aoa_elevation_FOM);

        NXPLOG_APP_D(
            "CCC.antenna_pair_info.configMode              : %d", cccRangingData->antenna_pair_info.configMode);
        NXPLOG_APP_D(
            "CCC.antenna_pair_info.antPairId1              : %d", cccRangingData->antenna_pair_info.antPairId1);
        NXPLOG_APP_D(
            "CCC.antenna_pair_info.antPairId2              : %d", cccRangingData->antenna_pair_info.antPairId2);

        NXPLOG_APP_D("CCC.noOfPdoaMeasures                          : %d", cccRangingData->noOfPdoaMeasures);
        for (size_t i = 0; i < cccRangingData->noOfPdoaMeasures; i++) {
            ...
            NXPLOG_APP_D(
                "CCC.pdoaMeasurements.pdoaIndex                : %d", cccRangingData->pdoaMeasurements[i].pdoaIndex);
        }
        NXPLOG_APP_D("CCC.noOfRssiMeasurements                      : %d", cccRangingData->noOfRssiMeasurements);
        for (size_t i = 0; i < cccRangingData->noOfRssiMeasurements; i++) {
            NXPLOG_APP_D("CCC.cccRssiMeasurements.rssi_rx1                     : %d.%d",
                TO_Q_8_8(cccRangingData->cccRssiMeasurements[i].rssi_rx1));

            NXPLOG_APP_D("CCC.cccRssiMeasurements.rssi_rx2                     : %d.%d",
                TO_Q_8_8(cccRangingData->cccRssiMeasurements[i].rssi_rx2));
        }
        NXPLOG_APP_D("CCC.cccSnrMeasurements                      : %d", cccRangingData->noOfSnrMeasurements);
        for (size_t i = 0; i < cccRangingData->noOfSnrMeasurements; i++) {
            NXPLOG_APP_D("CCC.cccRssiMeasurements.slotIndexAndAntennaMap    : %hu", cccRangingData->cccSnrMeasurements[i].slotIndexAndAntennaMap);
            NXPLOG_APP_D("CCC.cccRssiMeasurements.snrFirstPath              : %hu", cccRangingData->cccSnrMeasurements[i].snrFirstPath);
            NXPLOG_APP_D("CCC.cccRssiMeasurements.snrMainPath               : %hu", cccRangingData->cccSnrMeasurements[i].snrMainPath);
            NXPLOG_APP_D("CCC.cccRssiMeasurements.snrTotal                  : %d.%d", TO_Q_8_8(cccRangingData->cccSnrMeasurements[i].snrTotal));
        }
    }
}
